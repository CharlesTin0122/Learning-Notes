...
- 知识库入口位于 `Notes/🏠 Home.md`，待办入口为 `Notes/📝 待写清单.md`；各主要领域使用 `MOC-*` 笔记组织导航，包括 Maya、Unreal、MotionBuilder、program、math、Misc、Work。
- 当前知识库规模约为 850+ 篇 Markdown 笔记、4000+ 个附件，属于大型 Vault。执行修改时应限定目标目录和文件，避免无意进行全库格式化、批量重命名或链接重写。
- 内容体量主要集中在 `Notes/program/`、`Notes/Unreal/`、`Notes/MAYA/`。处理问题时优先结合游戏动画绑定 / Tech Art 语境，而不是按通用软件项目理解。
- 笔记大量包含 Python、C++、C、Bash/PowerShell 等代码围栏；代码是学习示例和知识记录，不代表仓库存在可构建工程。
- 新增跨主题内容时，除放入合适目录外，还应检查对应 `MOC-*` 是否需要添加相对 Markdown 链接；不要自动改写历史笔记中的旧格式，除非用户明确要求整理。
