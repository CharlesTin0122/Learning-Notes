...
- **在[Set](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/set-containers-in-unreal-engine)或[Map](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/map-containers-in-unreal-engine)中用作键。弱指针可能会在未通知容器的情况下随时无效，因此共享指针或共享引用更适用于充当键。可安全地将弱指针用作数值。
...
void AMySmartPtrActor::MyWeakPtr()
{
	//声明弱指针
	TSharedPtr<FSmartPtrStruct> ObjectOwner = MakeShared<FSmartPtrStruct>();
	TWeakPtr< FSmartPtrStruct> ObjectObserver(ObjectOwner);

	//弱指针不会阻止对象被销毁。在范例中，无论 ObjectOwner 是否在范围内，重置 ObjectOwner 都将销毁对象：
	ObjectOwner.Reset();
	// ObjectObserver.Pin()将弱指针提升为共享指针，由于ObjectOwner已经被重置，对象应该已经被销毁，因此ObjectObserver.Pin()应该返回空指针，不会触发断言失败
	if (ObjectObserver.Pin())
	{
		check(false);
	}
	...
	//弱指针安全复制
	TWeakPtr<FSmartPtrStruct> AnotherObserver = ObjectObserver;

	//弱指针重置
	AnotherObserver = nullptr;
	AnotherObserver.Reset();

	/* Pin 函数将创建指向弱指针对象的共享指针。
	* 如果对象仍然有效（未被销毁），Pin() 会返回一个新的 TSharedPtr，这个 TSharedPtr 的构造会增加引用计数。
	* 如果对象已被销毁，Pin() 会返回一个空指针。引用计数不变为0，因为对象已经被销毁。
	* 当 Pin() 返回的 TSharedPtr 作用域结束并被销毁时，引用计数会相应减少 1。
	*/
