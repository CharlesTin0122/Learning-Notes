...
- 载入驱动骨骼和被驱动骨骼
- ![](attachments/e5e0a7c9b1c11acd2d8319ca3edcaf6b_MD5.png)
- 添加姿态目标，可以选择从姿势资产复制所有，如果在maya中使用 PoseWrangler插件，可使用 Pose Driver Connect虚幻插件导入数据
- ![](attachments/UE5次级动画解决方案-29.png)
- RBF设置， 可勾选自动半径
- ![](attachments/46912ca7722814b869121394dab5e859_MD5.png)
- 设置驱动源和驱动输出，可以选择Rotation或Translation来驱动曲线(混合变形)或姿势（骨骼变形和混合变形）
- 所有节点选项
- ![](attachments/2a4e49f59b9a97b22241e76e7e2261da_MD5.png)
#  Bone Driven Controller

![](./attachments/0.0-UE5次级动画处理-1782789074218.webp)
  - 这是 UE内置里**最接近"驱动关键帧"**的节点:用源骨骼的某个分量(旋转/平移/缩放)经过一条重映射曲线,去驱动目标骨骼的某个分量。
  - 支持自定义 Remap Curve(就是你的"驱动曲线")、乘子、范围 clamp。
  - 适合:门轴、活塞、辅助骨跟随、简单的"A 转动 → B 缩放/平移"这类一对一机械驱动
  - ![](./attachments/0.0-UE5次级动画处理-1782789114186.webp)