# 概述
- 使用动画后处理来实现次级动画的运动
- 创建动画蓝图，将动画蓝图添加到skeletal mesh。
- ![](attachments/UE5%20Secondary%20Animation%20in%20Maya%20and%20Unreal-2.png)
- 在动画蓝图中如果角色蓝图中该Mesh直接挂载了动画蓝图，则后处理动画蓝图可以使用input pose节点获取动画姿态
- ![](attachments/UE5%20Secondary%20Animation%20in%20Maya%20and%20Unreal-4.png)
- 如果在角色蓝图中该Mesh作为附加资产（比如服装附加到身体），则后处理动画蓝图需要使用copy pose from mesh节点获取动画姿态
- ![](attachments/UE5%20Secondary%20Animation%20in%20Maya%20and%20Unreal-3.png)
- 记得要勾选使用附加的父项
- ![](attachments/UE5%20Secondary%20Animation%20in%20Maya%20and%20Unreal-6.png)
- 总之，使用copy pose from mesh不会有问题，所以后处理动画蓝图一律使用copy pose from mesh节点获取姿态
# Spring Controller
