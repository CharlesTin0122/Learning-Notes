...
> **用 Scoop 安装 shell 环境**：`scoop install git` 会带来 Git Bash（用法见 [Git Bash 使用指南](Git%20Bash%20使用指南.md)）；`scoop install nu` 安装 Nushell（用法见 [Nushell 使用指南](Nushell%20使用指南.md)）。两者都由 Scoop 统一升级，`scoop update nu git` 即可。
