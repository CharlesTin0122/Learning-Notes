...
- ![](attachments/03-%20Control%20Creation%20&%20Setup.png)
- 然后再使用控制器来约束骨骼。
# 控制器的合并
- 我们有两个控制器，想要将其合并为一个
- ![](attachments/03-%20Control%20Creation%20&%20Setup-1.png)
- outline中开启shapes，将控制器分解为transform和shape两部分
- 选择方向控制器的shape节点和圆形控制器的transform节点
- ![](attachments/03-%20Control%20Creation%20&%20Setup-2.png)
- 如果直接执行父子关系，会导致子对象多出一个transform节点
- ![](attachments/03-%20Control%20Creation%20&%20Setup-5.png)
- 运行Python：`pm.parent(shape = True, relative = True)`，得到结果
- ![](attachments/03-%20Control%20Creation%20&%20Setup-3.png)
- outline中关闭shapes
- ![](attachments/03-%20Control%20Creation%20&%20Setup-4.png)
# 添加属性
- 添加属性
...
# 修改控制器颜色
![](attachments/03-%20Control%20Creation%20&%20Setup-7.png)
# 控制器影响另一个控制器形状
![](attachments/03-%20Control%20Creation%20&%20Setup-8.png)
- 将大控制器的部分顶点创建Cluster，并作为小控制器子对象，隐藏Cluster。就可以实现控制器影响另一个控制器形状
- ![](attachments/03-%20Control%20Creation%20&%20Setup-9.png)
# 控制器标签和上下选取控制器
![](attachments/03-%20Control%20Creation%20&%20Setup-10.png)
...
![](attachments/03-%20Control%20Creation%20&%20Setup-11.png)
- 代码
```python
