...
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank.png)
- 为脚部IK添加属性
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-1.png)
- 现在腿部的IKFK骨骼也要包含爪子骨骼用于切换，父子约束主骨骼。且IK骨骼要新增脚趾末端骨骼和脚底骨骼用于IK制作
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-2.png)![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-3.png)
# 绑定FK骨骼
- 给FK骨骼增加控制器，并父子约束FK骨骼，将控制器层级放置于脚部FK控制器之下。
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-4.png)
# 绑定IK骨骼
- 创建反转脚骨骼，记得冻结变换骨骼
- ![](初级/attachments/2e8538f2d5a0aa02802a97f6fb9517a0_MD5.jpeg)![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-5.png)
- 创建单链IK
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-6.png)
- 为每个脚趾IKHandle创建偏移组，可以使用脚本实现
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-7.png)
```python
...
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-8.png)
...
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-9.png)![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-10.png)
- 将所有的脚趾IKHandle转移到l_rear_ball_rev层级下
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-11.png)![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-12.png)
- 所有的脚趾尖IKHandle转移到反转脚l_rear_tip_rev层级之下
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-13.png)
- 最后，将新增骨骼的IKHandle放置于l_rear_backcarpus_rev层级之下
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-14.png)
- 为脚趾每一个IKHandle打一个组，并将组的轴心移动到上一节骨骼的位置，该组用于脚趾关节的旋转
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-15.png)
## 链接控制器和反转脚
## Roll
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-16.png)
...
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-17.png)
	- multiplyDivide10：取反，*=-1
	- condition6：判断控制器X轴位移大于0。是，则返回旋转值和0。否，则返回0和旋转值
## 链接控制器和属性
- 实现Heeltwist和ToeTwist
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-18.png)
- 实现ToeTap和ToeCurl
- 使用控制器属性链接的是IKHandle层级制上的旋转偏移组
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-22.png)
- 实现ToeSpread属性，脚趾张开
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-23.png)
	- 使用paw spread属性链接脚趾IKHandle的X轴位移，使得他们向两侧移动，
	- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-24.png)
	- 两侧移动幅度是+-1，中间两条是+-0.5
## 限制控制器
- 使用控制器属性中的Limit来限制控制器
- ![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-25.png)
## 修改rear_leg_l_hock_ctrl的约束
rear_leg_l_hock_ctrl之前是被脚部IK控制器约束的，现在我们把约束节点删掉，用反转脚末端骨骼点约束rear_leg_l_hock_ctrl控制器偏移组
![](初级/attachments/2.07-Paw%20Roll%20&%20Bank-26.png)