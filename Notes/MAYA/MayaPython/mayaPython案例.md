...
# 添加影响
pm.skinCluster(
	skincluster, edit=True, addInfluence=jnt, weight=0.0, lockWeights=True
)
# 移除影响
pm.skinPercent(skincluster, v, transformValue=[(jnt, 0.0)], normalize=False)
pm.skinCluster(skincluster, edit=True, removeInfluence=jnt)
```
## **绑定姿态**
```python
import pymel.core as pm
...
## 烘焙动画
```python
def bake_jonts(all_joints, start_frame, end_frame):
    """烘焙动画,烘焙完成后要删除烘焙骨骼所连接的约束节点

    Args:
        all_joints (list): 要烘焙的骨骼列表
        start_frame (int): 烘焙起始帧
        end_frame (int): 烘焙结束帧
    """
    pm.bakeResults(
        all_joints,
    ...
    # 关键步骤：烘焙后断开所有约束，防止控制器继续干扰骨骼
    for jnt in all_joints:
        constraints = pm.listConnections(jnt, type="constraint")
        if constraints:
...
def add_namespace_to_objects(objects, spacename):
    ...
    :param objects:   需要添加命名空间的对象列表
    :param spacename: 命名空间名称
    """
    import pymel.core as pm

    if not objects:
        pm.warning("objects 列表为空，操作终止。")
        return

    if not spacename or not isinstance(spacename, str):
        pm.warning("spacename 必须是一个非空字符串，操作终止。")
        return

    # ==================== 创建命名空间 ====================
    # 如果该命名空间已经存在，则不重复创建，直接复用
    if not pm.namespace(exists=spacename):
        pm.namespace(add=spacename)
        print(f"命名空间 '{spacename}' 创建成功。")
    else:
        print(f"命名空间 '{spacename}' 已存在，跳过创建。")
    ...
    for obj in objects:
        try:
            # 将对象转为 PyNode，兼容字符串和 PyNode 两种输入
            node = pm.PyNode(obj)

            # 获取对象当前的短名称（去掉路径和旧命名空间）
            # 例如: "oldNS:spine_01" -> "spine_01"
            short_name = node.nodeName().split(":")[-1]

            # 拼接新名称：命名空间:短名称
            new_name = f"{spacename}:{short_name}"

            pm.rename(node, new_name)
            success_list.append(new_name)

        except Exception as e:
            failed_list.append((str(obj), str(e)))

    # ==================== 打印结果报告 ====================
    print(f"\n====== 添加命名空间完成 ======")
    print(f"成功: {len(success_list)} 个对象")

    if failed_list:
        print(f"失败: {len(failed_list)} 个对象")
...
## 移除名称空间
```python
def remove_namespace(spacename, move_to_root=True):
    """
    删除指定的命名空间
    
    :param spacename:    要删除的命名空间名称
    :param move_to_root: True  = 将命名空间下的对象移回根命名空间（保留对象）
                         False = 连同命名空间下的所有对象一并删除（危险操作）
    """
    import pymel.core as pm
    ...
    if not pm.namespace(exists=spacename):
        pm.warning(f"命名空间 '{spacename}' 不存在，操作终止。")
        return False

    # ==================== 处理命名空间下的对象 ====================
    if move_to_root:
        # 安全模式：将所有对象移回根命名空间 ":"
        # mergeNamespaceWithRoot 会自动处理该命名空间下的全部对象
        pm.namespace(removeNamespace=spacename, mergeNamespaceWithRoot=True)
        print(f"命名空间 '{spacename}' 已删除，对象已移回根命名空间。")
    else:
        # 危险模式：删除命名空间及其下的所有对象
        # 先查出所有属于该命名空间的节点，强制删除
        nodes_in_ns = pm.namespaceInfo(spacename, listNamespace=True) or []
        if nodes_in_ns:
            pm.delete(nodes_in_ns)
            print(f"已删除 {len(nodes_in_ns)} 个属于 '{spacename}' 的对象。")
        # 此时命名空间已为空，可以直接删除
        pm.namespace(removeNamespace=spacename)
        print(f"命名空间 '{spacename}' 及其所有对象已被彻底删除。")
...
| :------------------------ | :--- | :------------------------------------------------------- |
| QT_PREFERRED_BINDING_JSON | str  | 覆盖绑定顺序和内容以进行尝试。这可以应用于每个 Qt.py 命名空间。                      |
| QT_PREFERRED_BINDING      | str  | 覆盖绑定的顺序和内容以供尝试。在 QT_PREFERRED_BINDING_JSON 不适用时使用        |
| QT_VERBOSE                | bool | Be a little more chatty about what's going on with Qt.py |
## Qt.py所调用PySide版本
- Qt.py 模块会根据 QT_PREFERRED_BINDING 环境变量去检索PySide版本来调用
- 从maya2025之后，maya移除了 PySide2 和 shiboken2 ，全面转向 PySide6 和 shiboken6
- 但因为一些遗留原因导致maya的QT_PREFERRED_BINDING环境变量始终为`QT_PREFERRED_BINDING: PySide2;PySide2`
- 导致Qt.py 模块失效，报错：Qt.py line 2336: No Qt binding were found.
- 因此我们将环境变量QT_PREFERRED_BINDING移除，或修改为`QT_PREFERRED_BINDING:"PySide6;PySide2;PyQt6;PyQt5"`让Qt.py 模块依次检索调用。
```python
import os
# 查询QT_PREFERRED_BINDING变量
print("QT_PREFERRED_BINDING:", os.getenv("QT_PREFERRED_BINDING"))
print("QT_API:", os.getenv("QT_API"))
# 查询PySide版本
try:
    from PySide6 import QtCore
    print("PySide6 可以 import")
except ImportError:
    print("PySide6 無法 import")

try:
    from PySide2 import QtCore
    print("PySide2 可以 import")
except ImportError:
    print("PySide2 無法 import")
# 查询python版本
import sys
print("sys.version:", sys.version)


# 清掉舊的強制設定，然後明確指定 PySide6优先
os.environ.pop("QT_PREFERRED_BINDING", None)   # 先移除舊值
os.environ["QT_PREFERRED_BINDING"] = "PySide6;PySide2;PyQt6;PyQt5"
# 或者直接移除改变量
if 'QT_PREFERRED_BINDING' in os.environ:
    del os.environ['QT_PREFERRED_BINDING']
    print("已清除 QT_PREFERRED_BINDING 环境变量")
```
# maya设置
