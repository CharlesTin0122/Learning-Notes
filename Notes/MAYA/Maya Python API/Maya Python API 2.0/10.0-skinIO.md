...
class SkinClusterIO:
    ...
    def __init__(self, obj_list=None):
        ...
        if not obj_list:
            raise RuntimeError("No objects provided or selected.")
    ...
    def export_skin(self, file_path=None, *args):
    ...
    def import_skin(self, file_path=None, *args):
        ...
        for data in data_pack["objDDic"]:
            ...
            try:
                ...
                pm.displayWarning(warningMsg.format(obj_name))
    ...
    def set_blend_weights(self, skinCluster, dagPath, components, data, compressed):
        if compressed:
            # json格式压缩会跳过0权重，如果key为空，则将其值设为0.0
            # json的key不能是整数，只能是字符串 vtx[35]：0.69将是: u35: 0.69
            blendWeights = om.MDoubleArray(data["vertexCount"])
            ...
            for i, val in enumerate(data["blendWeights"]):
                blendWeights[i] = val
        skin_fn = self.get_skinCluster_fn(skinCluster.name())
        skin_fn.setBlendWeights(dagPath, components, blendWeights)
